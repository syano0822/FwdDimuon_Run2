...
void runAnalysisQA(TString runPeriod = "LHC16k",
		   ...
		   bool isMix        = false)
{
  ...
  AliAnalysisTaskAODEventStudy *task
    = reinterpret_cast<AliAnalysisTaskAODEventStudy*>(gInterpreter->ExecuteMacro(Form("AddTaskAODEventQA.C(%u, %f, %f, %d, %f, %f, \"%s\", %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,%d,%f,%d)",
										     offlineTriggerMask,
										     min_vtxz,
										     max_vtxz,
										     min_vtx_cont,
...
AliAnalysisGrid* CreateAlienHandler(TString runPeriod, TString run_mode, Bool_t isJDL, TString type, bool onMixingAnalysis){
  ...
  if (onMixingAnalysis) {
    plugin->SetGridWorkingDir("PWGLF/AOD/"+runPeriod+"/LowMassDimuonMix/"+type);
  } else {
