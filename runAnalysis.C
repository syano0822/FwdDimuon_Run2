...
void runAnalysis(string runPeriod = "LHC16k",
		 ...
		 bool isMix        = false)
{
  ...
  AliAnalysisTaskAODTrackPair *task
    = reinterpret_cast<AliAnalysisTaskAODTrackPair*>(gInterpreter->ExecuteMacro(Form("AddTaskAODTrackPair.C(%u, %f, %f, %d, %f, %f, \"%s\", \"%s\", %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,%d,%f,%d,%d)",
										     offlineTriggerMask,
										     min_vtxz,
										     max_vtxz,
										     min_vtx_cont,
...
AliAnalysisGrid* CreateAlienHandler(string runPeriod, string run_mode, Bool_t isJDL, string type, bool onMixingAnalysis){
  ...
  if(type == "LHC20f10c"){
    ...
    if (runPeriod.find("LHC16i") != std::string::npos){
